      ...
      <div className="right-container" style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '20px' }}>
        ...
        <div className="powerups-list" style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {Array.from(activePowerups.entries()).map(([type, time]) => (
            <div key={type} className="glass-panel-hud" style={{ padding: '10px 16px', display: 'flex', alignItems: 'center', gap: '12px', minWidth: '160px' }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: '0.8rem', textTransform: 'uppercase', fontWeight: 'bold' }}>{type.replace('_', ' ')}</div>
                <div style={{ width: '100%', height: '4px', background: 'rgba(255,255,255,0.1)', borderRadius: '2px', marginTop: '6px', overflow: 'hidden' }}>
                  <div style={{ width: `${(time / (type === 'speed' ? 5 : 10)) * 100}%`, height: '100%', background: 'var(--primary-neon)', transition: 'width 0.1s linear' }} />
                </div>
              </div>
            </div>
          ))}
